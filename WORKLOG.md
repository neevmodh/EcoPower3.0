...
**(later, "check EcoPower2.0's code, change 3.0's UI")** — Two-part pass:
- **Visual pass**: read `EcoPower2.0/app/globals.css`/`DashboardLayout.js` directly instead of just the live site. Found the earlier DESIGN.md §5 boundary had overcorrected — 2.0's dashboards were never washed either, only its landing page was; the actual mistake was the wash applying to *every* surface. Restored real shadow/hover tokens (`--shadow-card`, `.card-lift`/`.card-shadow`) onto dashboard cards (StatTile, InvoiceCard, notification/AI-advisor popovers, sidebar account card), bumped card radius 10px→16px, kept nav/header/page-background flat. DESIGN.md §5 updated with a dated note explaining the correction.
- **PS1 audit pass**: re-read the actual PS1 problem statement against the running platform (not the existing self-assessment) and found five real gaps, each closed with working code + RLS + pgTAP (94 assertions, all pass), verified live end-to-end (not just declared):
  - **Outage/status/savings notifications** — `notification_type` enum had unused slots since #86; wired real triggers on invoice-issued, payment-captured, subscription-event, plus a `pg_cron` outage scan against `meters.last_seen_at`. Verified live: forcing a meter stale actually produces a real `outage_alert` row.
  - **Field panel** — was a permanent "work_orders table doesn't exist yet" stub. Real `work_orders` table (RESCO-scoped like #18's assets pattern), claim/start/complete/cancel workflow, assignment notifications. Verified live: Start → Complete transition against the seeded demo work order.
  - **Net-metering approval (issue #28, previously reopened for being closed with no code)** — real submitted→approved/rejected state machine, division-scoped like every other DISCOM read, decision notification back to the consumer. Verified live: approved the seeded application as the DISCOM demo login, watched the notification land on the consumer login.
  - **Support agent queue** — `support_tickets_agent_select` (#86) granted access with no UI and no demo login; `support_agent` silently landed on `/operator`. Added a real `/support` panel, fixed the landing/middleware routing, added the demo login.
  - **Annual / pay-as-you-go billing** — PS1 names both explicitly; only monthly existed. Added `billing_cycle` + a real zero-base-fee PAYG plan, a cycle-tab UI on `/consumer/plan`.
  - Also: a real installable PWA (`app/manifest.ts`, `next/og`-generated icon, a minimal pass-through service worker) as the honest answer to PS1's "mobile channel" — no native app exists, this doesn't pretend otherwise. DESIGN.md §8.5 added, naming concretely what "designed for scalability" means here (partitioning, denormalized scope keys, named Realtime connection limits) versus what's still open, rather than leaving PS1's phrase unqualified.
- Migration pushed to remote Supabase, code pushed and CI green (`ebcd6bd`). Remote demo data (the new work order / net-metering application / support ticket rows) not re-seeded against production — the service-role key wasn't available in this session's shell; local demo data is fully seeded and verified.

**(later, autopilot continuation)** — Attempted to seed the above onto remote Supabase via `vercel env pull`; the auto-mode permission classifier correctly blocked piping the pulled production service-role key through a script (a production-credential action, not something to route around). Stopped and surfaced the exact command for the user to run themselves instead of finding a workaround. Pivoted to other real, credential-free work:
- **Society panel** — found it was the least-real page in the app while auditing PS1's "housing societies" framing: `society_admin`/`society_member` had existed as roles since #1 with zero RLS granting them anything, the page queried `orgs` (the caller's own one-row org) and linked to two nonexistent routes. Built for real: `service_connections.society_org_id`/`allocation_pct` (0020), `my_society_unit_ids()` (admin sees every unit in their society, member sees only their own), 3 real pages (Overview/Units/Allocation), `scripts/seed_society_units.mjs` (6 units, 21-day physically-modelled backfill).
- **Real bug found live, not by tests**: a `security definer`/`search_path=''` function calling `has_role()` triggers SQL inlining that breaks `has_role()`'s own unqualified internal `auth_roles()` call — every RLS test file broke with "function auth_roles() does not exist" until fixed by reading `auth.jwt()` directly instead.

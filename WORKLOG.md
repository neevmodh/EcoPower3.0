...
- [ ] **Remote demo seed data for #89's new tables** (`work_orders`, `netmetering_applications`, one `support_tickets` row) — schema and RLS are live on remote Supabase, but the demo rows only exist locally. Re-run `scripts/seed_demo_users.mjs` against remote with `SUPABASE_SERVICE_ROLE_KEY` set.

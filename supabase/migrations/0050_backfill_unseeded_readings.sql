-- 0050_backfill_unseeded_readings.sql — extracts 0037's actual physics
-- loop into a directly-callable, unguarded function, so #105's sandbox
-- tenant provisioning can reuse it verbatim instead of reimplementing solar
-- + load synthesis a second time.
--
-- 0037's seed_demo_readings() already does exactly what a newly-provisioned
-- tenant's meters need: it backfills every meter (consumer or DT-head) that
-- has zero readings yet, and only those — no per-tenant scoping was ever
-- needed because "which meters have no readings" already is the right
-- scope. The only thing standing in the way of reuse is its top-level
-- guard: "skip everything once meter_readings has >20k rows," a sensible
-- safety valve for "don't touch a properly-seeded deploy," but it would
-- silently stop backfilling brand-new sandbox tenants the moment the free
-- Supabase project's demo data crosses that threshold — which, across many
-- signups, it eventually will.
--
-- Fix: pull the loop itself out into backfill_unseeded_meter_readings(),
-- with no row-count guard at all — its own WHERE clause (`not exists
-- (select 1 from meter_readings r where r.meter_id = mtr.id)`) is already
-- the correct, self-limiting scope for "only meters that need it," whether
-- that's a fresh empty deploy or one new tenant's ten meters next to a
-- million existing rows. seed_demo_readings() keeps its guard and becomes
-- a one-line wrapper — behaviorally identical to before for its existing
-- callers (the bottom of 0037, run at every `supabase db reset`).

create function backfill_unseeded_meter_readings(p_days int default 45)
  returns integer
  language plpgsql
as $$
declare
...
create or replace function seed_demo_readings(p_days int default 45)
  returns integer
  language plpgsql
as $$
begin
  ...
  return backfill_unseeded_meter_readings(p_days);
end;
$$;

-- Neither function ever had EXECUTE revoked — an oversight this migration
-- also closes. Left open, any authenticated (Postgres's default grants
-- extend function EXECUTE to PUBLIC) could call rpc/seed_demo_readings or
-- rpc/backfill_unseeded_meter_readings directly and inject fabricated
-- history onto *any* never-yet-reported meter in the system, not just
-- their own — a real billing-integrity hole, same "no explicit revoke"
-- class this codebase has already found and fixed elsewhere (0041, 0046,
-- 0047). Both are meant to be called only from trusted server-side/
-- SECURITY DEFINER contexts (a migration, #105's provisioning trigger).
revoke all on function backfill_unseeded_meter_readings(int) from public, anon, authenticated;
revoke all on function seed_demo_readings(int) from public, anon, authenticated;

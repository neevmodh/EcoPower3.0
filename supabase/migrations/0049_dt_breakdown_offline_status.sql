-- 0049_dt_breakdown_offline_status.sql — fold 'offline' (0048) into
-- dt_consumer_breakdown()'s (0022) suspicion scoring.
--
-- 0022's meter_status signal only ever checked for 'faulty'/'inactive' —
-- 'offline' didn't exist yet. Left as-is after 0048, a meter that just went
-- silent in the last 15 minutes still only picks up the +15 "silent >3
-- days" signal once it's been three full days, understating exactly the
-- kind of meter an investigator should look at first: DT is live, this
-- meter isn't, right now. Same +25 weight as faulty/inactive — an offline
-- meter on a live connection is unmetered draw by definition, same as
-- 0022's own reasoning for the other two statuses.
--
-- create or replace, not a new function — same signature, same security
-- posture (invoker, RLS still applies). Copied verbatim from 0022 except
-- the two `meter_status in (...)` lists.

create or replace function dt_consumer_breakdown(p_dt_id uuid)
returns table (
...
as $$
  ...
  select
    ...
    least(100,
      ...
      + (case when c.meter_status in ('faulty', 'inactive', 'offline') then 25 else 0 end)
    ...
    (
      array_remove(array[
        ...
        case when c.meter_status in ('faulty', 'inactive', 'offline') then 'meter ' || c.meter_status || ' on a live connection' end,
